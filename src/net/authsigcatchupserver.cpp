// SEADS authoritative ASYMMETRIC-SIGNATURE + ASYNC + CATCH-UP server (netcode layer 29). See
// authsigcatchupserver.h.
//
// This is broadcast_authsig_async's async loop (authsigasyncserver.cpp) with broadcast_auth_catchup's
// history-retention + windowed catch-up replay folded in — a SIBLING so the sealed broadcast.cpp AND every
// prior server are byte-for-byte untouched. The downstream helpers mirror authsigasyncserver.cpp's static
// helpers verbatim; PubkeyTable / seat_authorizes / CHALLENGE-001 / HELLO-003 / BIND-001 are reused
// verbatim from layers 27/18. Two things differ from layer 28:
...
// One connected client. Identical to authsigasyncserver.cpp's AuthSigAsyncClient — catch-up adds no
// per-client state (the retained history lives on the server, not the client).
struct AuthSigCatchupClient {
    netsock::socket_t s;
...
bool flush_client(AuthSigCatchupClient& c) {
...
bool over_cap(const AuthSigCatchupClient& c, std::size_t cap_bytes) {
    return cap_bytes > 0 && c.pending_bytes() > cap_bytes;
}

bool enqueue_bytes(AuthSigCatchupClient& c, const std::vector<std::uint8_t>& bytes) {
    c.buf.insert(c.buf.end(), bytes.begin(), bytes.end());
    return flush_client(c);
}

void drop_client(std::vector<AuthSigCatchupClient>& clients, std::size_t i, Stats& st, PubkeyTable& creds) {
...
void reap_dead(std::vector<AuthSigCatchupClient>& clients, Stats& st, std::size_t liveness_frames,
               PubkeyTable& creds) {
    if (liveness_frames == 0) return;
    for (std::size_t i = clients.size(); i-- > 0;) {
        AuthSigCatchupClient& c = clients[i];
...
Stats broadcast_authsig_catchup(netsock::socket_t listener, InputProducer& producer, CommandQueue& queue,
    ...
    Stats st;
    std::vector<AuthSigCatchupClient> clients;
    ...
    // HELLO-003 (bounded-blocking); (c) VERIFY the signature to a seat (or spectator); (d) go non-blocking
    // and ENQUEUE the BIND-001; (e) — the layer-20/29 addition — ENQUEUE the retained catch-up prefix right
    // after the BIND. A joiner shed during replay by the byte-cap is `capped`, its seat returned, never a
    // member. Command frames pipelined after the HELLO are submitted once the joiner is confirmed a member.
    auto accept_all = [&]() {
        while (true) {
            netsock::socket_t c = netsock::accept_one(listener);
            if (!netsock::is_valid(c)) break;
            AuthSigCatchupClient ac;
            ac.s = c;
    ...
    if (clients.size() < min_initial) {
        for (AuthSigCatchupClient& c : clients) netsock::close_socket(c.s);
        return st;  // ok stays false
    }

    // --- bidirectional frame loop (authcatchupserver.cpp's loop verbatim) -----------------------
    ...
    for (std::size_t fi = 0;; ++fi) {
        ...
        rfds.clear();
        wfds.clear();
        for (const AuthSigCatchupClient& c : clients) {
            rfds.push_back(c.s);
    ...
    // --- bounded DRAIN (authcatchupserver.cpp's tail verbatim) ----------------------------------
    int idle = 0;
    while (idle < 600) {
        rfds.clear();
        wfds.clear();
