...
//
// Usage:  seads_netserver [port] [num_clients] [catchup]
...
int main(int argc, char** argv) {
    ...
    std::printf("seads_netserver: listening on 127.0.0.1:%u (%zu frames ready, expecting %d client(s), "
                "catchup=%d)\n",
                port, frames.size(), num_clients, catchup ? 1 : 0);
    std::fflush(stdout);
    ...
    // layer-9/10 single-thread select() broadcast: wait for num_clients, then stream, accepting any
    // late joiner (catchup? whole stream via prefix replay : just the suffix) and dropping any
    // client that closes. 60 s bounded initial-wait.
