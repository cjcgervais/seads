// SEADS ASYMMETRIC-SIGNATURE + ASYNC + CATCH-UP determinism BRIDGE (netcode LAYER 29): the layer-27
// PUBLIC-KEY binding + the layer-16/19 async hygiene + broadcast_live's windowed late-join CATCH-UP, all
// composed — the LAST rung of the asymmetric-signature arc (27 -> 28 -> 29, the 22->23 step re-run on the
// signed credential). broadcast_authsig_catchup is a SIBLING of both broadcast_authsig_async (layer 28) and
// broadcast_auth_catchup (layer 23). This bridge proves, over real 127.0.0.1 sockets with NO sleeps / NO
// timing guesses, that the composition preserves ALL THREE claims:
//
//   LEG 1 (verified identity + catch-up window regimes — the headline): THREE identities present DISTINCT
//   tokens + correct seeds from the initial gather (token order != seat order: 100->2, 200->0, 300->1), are
//   CHALLENGED, SIGN, learn their seat from BIND, and upstream ONLY that seat's commands (SCRAMBLED +
//   chunked) — so the produced stream is BYTE-IDENTICAL to session::build_server_frames. A FOURTH client
//   joins mid-stream at frame kJoin with an UNKNOWN token (999 -> SPECTATOR) and — across W ∈ {1, kJoin/2,
//   retain-all} — receives EXACTLY [CHALLENGE | BIND(spectator) | frames[max(0,kJoin-W):]], trimmed ==
//   max(0, N-W). The three seated identities are byte-identical to the whole stream under EVERY window.
//
//   LEG 2 (verify + authorization compose with catch-up): the mid-stream SPECTATOR upstreams the WHOLE
//   command set; a spectator authorizes NOTHING, so every one is dropped (cmds_unauth == the whole set) and
...
//   its catch-up REPLAY backlog shed by the byte-cap (Stats.capped) + its SEAT (1) freed. DEAD's delivery is
...
// watchdog fails (not wedges). Exit 0 PASS, 1 FAIL. TRANSPORT-ONLY: broadcast_authsig_catchup is a SIBLING
// of broadcast_authsig_async + broadcast_auth_catchup (sealed broadcast.cpp + all prior servers untouched);
// CHALLENGE-001/HELLO-003/BIND-001 are transport metadata (no seal). Rides v1.26r0.
#include "authsigcatchupserver.h"
...
// ---------------------------------------------------------------------------------------------
// INPUT-SK-001: the SAME grid-exact (dyadic) 3-aircraft scenario the layer-15b..28 bridges use.
// ---------------------------------------------------------------------------------------------
...
// Read the CHALLENGE-001 (all[0]), sign it with `seed`, send HELLO-003 UP. True if challenge decoded + sent.
...
// =============================================================================================
// LEG 1 / LEG 2 harness: THREE seated identities (initial gather) + a mid-stream spectator (unknown token).
// Server runs broadcast_authsig_catchup(min_initial=3, cap=0, liveness=0, `window`). Clients read CHALLENGE
// first, sign, send HELLO-003, then read BIND (all[1]); frames = all[2:].
// =============================================================================================
...
static void run_catchup_leg(const Rails& rails, const session::Scenario& sc, std::size_t kJoin,
    ...
    std::thread server([&] {
        ...
        stats_out = netinput::broadcast_authsig_catchup(listener, producer, q, creds, SESSION_K0, SESSION_K1,
    ...
    for (std::size_t ci = 0; ci < 3; ++ci) {
        early.emplace_back([&, ci] {
            ...
            challenge_and_hello3(s, r, all, ROSTER[ci].token, make_seed(ROSTER[ci].base));
            std::uint8_t buf[4096];
    ...
    std::thread late([&] {
        ...
        framing::StreamReassembler r;
        std::vector<std::vector<std::uint8_t>> all;
        challenge_and_hello3(s, r, all, 999, make_seed(0xEE));  // unknown token -> spectator
        std::uint8_t buf[4096];
        while (all.size() < 2) {  // read until the BIND record (all[1]) is complete
            std::ptrdiff_t n = netsock::recv_some(s, buf, sizeof(buf));
            if (n <= 0) break;
    ...
    std::thread watchdog([&] {
        ...
        if (done.load() < expect_done) {
            std::printf("FAIL layer-29 authsig-catchup leg [%s] TIMED OUT (socket hang)\n", label);
            std::fflush(stdout);
            std::_Exit(1);
        }
    });
...
// ---------------------------------------------------------------------------------------------
// LEG 1: verified identity + catch-up window regimes.
// ---------------------------------------------------------------------------------------------
static int run_leg1() {
...
// =============================================================================================
// LEG 3: hygiene composes with identity + catch-up. FAST (token 200 -> seat 0, hook-drained) byte-identical
...
static int run_leg3() {
    ...
    std::thread server([&] {
        ...
        stats = netinput::broadcast_authsig_catchup(listener, producer, q, creds, SESSION_K0, SESSION_K1,
    ...
    std::thread watchdog([&] {
        for (int i = 0; i < 600 && done.load() < 3; ++i)
            std::this_thread::sleep_for(std::chrono::milliseconds(100));
        if (done.load() < 3) {
            std::printf("FAIL layer-29 hygiene leg TIMED OUT (wedge)\n");
            std::fflush(stdout);
            std::_Exit(1);
        }
    });
    ...
    // FAST: token 200 (seat 0). Prove the credential, read BIND, upstream ONLY its own seat's commands.
...
int main() {
    ...
    fails += run_leg3();
    if (fails == 0) {
        std::printf("PASS: layer-29 asymmetric-signature + async + catch-up server — the layer-27 PUBLIC-KEY "
                    "binding + the layer-16/19 async/byte-cap/liveness hygiene + layer-20 windowed late-join "
                    "CATCH-UP all compose: signed clients each fly their designated aircraft to "
                    "build_server_frames byte-for-byte, a mid-stream joiner catches up an exact window "
                    "suffix, an unknown-token spectator's commands are all rejected yet it still catches up, "
                    "and a dead authenticated client's replay backlog is shed by the byte-cap + its seat "
                    "freed\n");
        return 0;
    }
    std::printf("RESULT: layer-29 authsig-catchup bridge FAIL (%d mismatches)\n", fails);
    return 1;
}
