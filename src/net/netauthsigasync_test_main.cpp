// SEADS ASYMMETRIC-SIGNATURE + ASYNC determinism BRIDGE (netcode LAYER 28): the layer-27 PUBLIC-KEY binding
// merged with the layer-16/19 downstream OUTPUT HYGIENE (async send buffers / byte-cap / liveness reap) —
// the 21->22 step re-run on the SIGNED credential (27->28).
//
// broadcast_authsig_async is broadcast_authsig's challenge/signature verify driven through
// broadcast_auth_async's async loop — a SIBLING of both. This bridge proves, over real 127.0.0.1 sockets
// with NO sleeps / NO timing guesses, that the composition preserves BOTH claims:
//
//   LEG 1 (verified identity survives the async path — the merge anchor): THREE clients each hold a DISTINCT
//   token AND its correct PRIVATE seed (token order != seat order: 100->2, 200->0, 300->1), are CHALLENGED,
//   SIGN, learn their seat from BIND, and upstream ONLY that seat's commands (SCRAMBLED + chunked) while the
//   server runs the FULL async path (cap=0, liveness=0). Their union is the whole INPUT-SK-001 command set,
//   so each downstream — after CHALLENGE + BIND — is BYTE-IDENTICAL to session::build_server_frames.
//
//   LEG 2 (verify + authorization survive the async path): TWO clients upstream the WHOLE scenario. Client A
//   presents token 200 + its CORRECT seed -> seat 0 (foreign commands dropped); Client B presents token 300
//   but signs with the WRONG private key -> bad signature -> SPECTATOR (all its commands dropped — even
//   holding the whole public roster, an attacker cannot sign). Both see the aircraft-0-only world byte-for-byte.
//
//   LEG 3 (a dead AUTHENTICATED client is bounded WITHOUT disturbing the sim): TWO signed clients on a LONG
...
//   aircraft-0-only reference, its commands drove the sim. DEAD (token 300 -> seat 1) proves its credential
...
// watchdog fails (not wedges). Exit 0 PASS, 1 FAIL. TRANSPORT-ONLY: broadcast_authsig_async is a SIBLING of
// broadcast_authsig + broadcast_auth_async (sealed broadcast.cpp + all prior servers untouched);
// CHALLENGE-001/HELLO-003/BIND-001 are transport metadata (no seal). Rides v1.26r0.
#include "authsigasyncserver.h"
...
// ---------------------------------------------------------------------------------------------
// INPUT-SK-001: the SAME grid-exact (dyadic) 3-aircraft scenario the layer-15b..27 bridges use.
// ---------------------------------------------------------------------------------------------
...
// token -> (designated seat, private seed base). Token order != seat order. The server enrolls only the
// derived PUBLIC key.
struct Credential { std::int64_t token; std::int64_t seat; std::uint8_t base; };
static const Credential ROSTER[] = {{100, 2, 0x10}, {200, 0, 0x20}, {300, 1, 0x30}};

static void enroll_roster(netinput::PubkeyTable& creds) {
    for (const auto& c : ROSTER) {
        auto seed = make_seed(c.base);
        std::uint8_t pk[32];
        ed25519::public_key(seed.data(), pk);
...
// Client-side handshake: read the CHALLENGE-001 (the FIRST downstream framing frame, accumulated into
// `all` at index 0), sign the challenge with `seed`, and send HELLO-003 UP. Returns true if the challenge
// decoded and the HELLO was sent. Uses/extends the SAME reassembler `r` so the BIND record lands at all[1].
static bool challenge_and_hello3(netsock::socket_t s, framing::StreamReassembler& r,
...
// =============================================================================================
// LEG 1 / LEG 2 harness: N clients, each with a TOKEN + the private SEED it signs with (a forger's differs).
// The server runs broadcast_authsig_async(min_initial = N) through the FULL async path (cap=0, liveness=0).
// Per client: its decoded BIND + the snapshot frames it received (after CHALLENGE + BIND).
// =============================================================================================
struct ClientPlan {
    std::int64_t token;
...
static void run_leg(const Rails& rails, const session::Scenario& sc,
    ...
    std::thread server([&] {
        ...
        stats_out = netinput::broadcast_authsig_async(listener, producer, q, creds, SESSION_K0, SESSION_K1,
    ...
    for (std::size_t ci = 0; ci < N; ++ci) {
        clients.emplace_back([&, ci] {
            ...
            // 1) read CHALLENGE-001 (all[0]), sign, send HELLO-003.
            res.challenge_ok = challenge_and_hello3(s, r, all, plans[ci].token, plans[ci].seed);
    ...
    std::thread watchdog([&] {
        ...
        if (done.load() < expect_done) {
            std::printf("FAIL layer-28 authsig-async leg [%s] TIMED OUT (socket hang)\n", label);
            std::fflush(stdout);
            std::_Exit(1);
        }
    });
...
// ---------------------------------------------------------------------------------------------
// LEG 1: three identities each SIGN and fly the seat their TOKEN designates through the ASYNC path.
// ---------------------------------------------------------------------------------------------
static int run_leg1() {
    ...
    run_leg(rails, sc, plans, st, res, "leg1/3-identities-async");
    ...
    if (fails == 0)
        std::printf("  LEG1 PASS: 3 identities each SIGNED a fresh challenge and flew the seat their TOKEN "
                    "designates (100->2, 200->0, 300->1; scrambled/chunked) through the ASYNC path -> %zu "
                    "frames byte-identical to build_server_frames; zero unauthorized/capped/reaped\n",
                    ref.size());
    return fails;
}

// ---------------------------------------------------------------------------------------------
// LEG 2: a valid seat-0 client's foreign commands are dropped AND a FORGER (right token, wrong key)
// is a spectator (all commands dropped), through the async path; both see the aircraft-0-only world.
// ---------------------------------------------------------------------------------------------
static int run_leg2() {
    const Rails rails = sealed_rails();
    const session::Scenario sc = aa_scenario(/*ticks=*/150u);
    ...
    const std::size_t b_all = canon.size();                    // forger (spectator): all commands
    ...
    std::vector<ClientPlan> plans = {
        {200, make_seed(ROSTER[1].base), send_everything, /*chunk=*/1, /*reverse=*/true},  // valid -> seat 0
        {300, make_seed(0xBB), send_everything, /*chunk=*/5, /*reverse=*/false},           // FORGER -> spectator
    };

    netinput::Stats st;
    std::vector<ClientResult> res;
    run_leg(rails, sc, plans, st, res, "leg2/verify-boundary-async");
    ...
    if (fails == 0)
        std::printf("  LEG2 PASS: a seat-0 client's %zu foreign commands + a FORGER's %zu (right token, "
                    "wrong key -> spectator) all rejected (cmds_unauth=%zu) through the async path; both see "
...
// cooperative seated client. FAST (token 200 -> seat 0, hook-drained). DEAD (token 300 -> seat 1) proves
// its credential then reads/sends nothing. `use_liveness` selects the policy.
// =============================================================================================
static int run_hygiene_leg(bool use_liveness, const char* label) {
    ...
    std::thread server([&] {
        ...
        stats = netinput::broadcast_authsig_async(listener, producer, q, creds, SESSION_K0, SESSION_K1,
    ...
    // DEAD: token 300 (seat 1). Prove the credential (read CHALLENGE, sign, send HELLO-003), then read
    // nothing until the broadcast returns, then drain its kernel-buffered prefix to EOF — reassembling ALL
    // downstream bytes (incl. the CHALLENGE consumed during the handshake) through ONE reassembler so
    // dead_recs = [CHALLENGE, BIND, frame prefix...].
    ...
    std::thread dead([&] {
        ...
        if (netsock::is_valid(s)) {
            framing::StreamReassembler r;
            std::vector<std::vector<std::uint8_t>> all;
            challenge_and_hello3(s, r, all, 300, make_seed(ROSTER[2].base));  // valid identity -> seat 1
            {
                std::unique_lock<std::mutex> lk(mtx);
    ...
    std::thread watchdog([&] {
        for (int i = 0; i < 600 && done.load() < 2; ++i)
            std::this_thread::sleep_for(std::chrono::milliseconds(100));
        if (done.load() < 2) {
            std::printf("FAIL layer-28 hygiene leg [%s] TIMED OUT (wedge)\n", label);
            std::fflush(stdout);
            std::_Exit(1);
        }
    });
    ...
    // FAST: token 200 (seat 0). Prove the credential, read BIND, upstream ONLY seat-0 commands.
    ...
    std::vector<std::vector<std::uint8_t>> fast_all;
    {
        ...
        if (port != 0) {
            netsock::socket_t s = netsock::connect_loopback(port);
            if (netsock::is_valid(s)) {
                challenge_and_hello3(s, fast_rx, fast_all, 200, make_seed(ROSTER[1].base));
                std::uint8_t buf[4096];
...
int main() {
    ...
    if (fails == 0) {
        std::printf("PASS: layer-28 asymmetric-signature + async server — the layer-27 PUBLIC-KEY binding "
                    "(CHALLENGE-001 + verified HELLO-003 signature + BIND-001 + authorization) composes with "
                    "the layer-16/19 async/byte-cap/liveness downstream hygiene: signed clients each fly their "
                    "designated aircraft to build_server_frames byte-for-byte through the async path, a forger "
                    "gets no seat, and a dead authenticated client is bounded + its seat freed without "
                    "disturbing a cooperative client\n");
        return 0;
    }
    std::printf("RESULT: layer-28 authsig-async bridge FAIL (%d mismatches)\n", fails);
    return 1;
}
