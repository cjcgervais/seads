// SEADS BOUND + ASYNC determinism BRIDGE (netcode LAYER 19): layer-18 seat binding merged with the
// layer-16 downstream OUTPUT HYGIENE (async send buffers / byte-cap / liveness reap).
//
// Layer 18 (broadcast_bound) gave each client its own aircraft (a join-order SeatPolicy + a BIND-001
// handshake + upstream authorization) but kept broadcast_input's BLOCKING downstream. Layer 16
// (broadcast_bidi) gave the UNBOUND bidirectional server the async/byte-cap/liveness hygiene. Layer 19
// (broadcast_bound_async) is their product — a bound server with the async hygiene — the two axes being
// orthogonal. This bridge proves, over real 127.0.0.1 sockets with NO sleeps / NO timing guesses, that
// the composition preserves BOTH claims:
//
//   LEG 1 (binding survives the async path — the merge anchor): THREE clients each learn their seat from
//   the BIND handshake and upstream ONLY their own seat's commands (SCRAMBLED + adversarially chunked)
//   while the server runs the FULL async path (non-blocking per-client send buffers, cap=0, liveness=0).
//   Their union is the whole INPUT-SK-001 command set, so each client's downstream — after its BIND
//   record — is BYTE-IDENTICAL to session::build_server_frames, regardless of seat permutation or upstream
//   reorder/chunking. Distinct BIND seats in [0,3); zero unauthorized. (This is layer-18 LEG 1 driven
//   through the layer-16 async send path.)
//
//   LEG 2 (authorization survives the async path): ONE client seated at 0 upstreams the WHOLE scenario's
//   commands; the server accepts only its own-seat commands and DROPS the rest (cmds_unauth), so the
//   downstream is the aircraft-0-only world byte-for-byte — the foreign commands changed nothing, through
//   the async buffers.
//
//   LEG 3 (a dead client is bounded WITHOUT disturbing the sim — hygiene composes with binding): TWO
//   clients on a LONG scenario through a pinned tiny kernel send buffer. FAST is seated, upstreams ITS
//   OWN seat's commands once, and is drained every frame by the on_frame hook => never dropped, its
//   downstream (after BIND) byte-identical to the reference for the seat it drew, and its commands drove
//   the sim. DEAD connects, reads/sends nothing => it is dropped by the policy under test (liveness reap
//   at cap=0, sub-leg A; byte-cap shed at liveness=0, sub-leg B) and its SEAT is freed (leaves==1). The
//   drop changes nothing about FAST's bytes or the produced frames. DEAD's delivered bytes are its BIND
//   record followed by a strict prefix of the frame stream — the async buffer delivered the handshake FIRST.
//
...
//
// TRANSPORT-ONLY: no kernel/det_math/rails/wire/golden touched; broadcast_bound_async is a SIBLING of
// broadcast_bound + broadcast_bidi (sealed broadcast.cpp + both untouched); BIND-001 is transport
// metadata (layer-7 framing-envelope category, NOT a sealed rails.wire block) => no seal. Rides v1.26r0.
...
// ---------------------------------------------------------------------------------------------
// INPUT-SK-001: the SAME grid-exact (dyadic) 3-aircraft scenario the layer-15b/16/17/18 bridges use, so a
...
// ---------------------------------------------------------------------------------------------
static const session::Phase BA_P0[] = {
    {0u,   0.0,  1.0, 0.75, true},
    {50u,  0.5,  1.5, 1.0,  false},
    {120u, 0.0,  1.0, 0.75, false},
};
static const session::Phase BA_P1[] = {
    {0u,  -0.5,  1.0, 0.5,  false},
    {80u,  0.25, 2.0, 1.0,  true},
};
static const session::Phase BA_P2[] = {
    {0u,   0.5,  1.5, 1.0,  false},
};

static session::AircraftSpec BA_AC[3];
static const session::Scenario ba_scenario(unsigned ticks) {
    static bool built = [] {
        for (int i = 0; i < 3; ++i) BA_AC[i] = sess_vec::AIRCRAFT[i];  // envelope + start state
        BA_AC[0].sched = BA_P0; BA_AC[0].n_phase = 3;
        BA_AC[1].sched = BA_P1; BA_AC[1].n_phase = 2;
        BA_AC[2].sched = BA_P2; BA_AC[2].n_phase = 1;
        return true;
    }();
    (void)built;
    session::Scenario sc = {BA_AC, 3u, ticks, /*snap_every=*/5u, /*lag=*/0u, /*render=*/0u,
...
// =============================================================================================
// LEG 1 / LEG 2 harness: N clients, each described by a plan that — GIVEN the seat the server assigns it
// (learned from its BIND handshake) — produces the commands it upstreams, plus a scramble/chunking. The
// server runs broadcast_bound_async(min_initial = N) through the FULL async path (cap=0, liveness=0).
...
static void run_bound_async_leg(const Rails& rails, const session::Scenario& sc,
    ...
    std::thread server([&] {
        ...
        stats_out = netinput::broadcast_bound_async(listener, producer, q, /*min_initial=*/N,
    ...
    for (std::size_t ci = 0; ci < N; ++ci) {
        clients.emplace_back([&, ci] {
            ...
            // 1) read until the BIND handshake (the first framing frame) is complete.
            while (all.empty()) {
            ...
            // 2) build this client's commands from the seat the server assigned it, scramble, send UP.
            ...
            cv.notify_all();
            // 3) read the downstream snapshots to EOF (frames after the BIND).
            while (true) {
                std::ptrdiff_t n = netsock::recv_some(s, buf, sizeof(buf));
                if (n <= 0) break;
                if (!r.feed(buf, static_cast<std::size_t>(n), all)) break;
    ...
    std::thread watchdog([&] {
        ...
        if (done.load() < expect_done) {
            std::printf("FAIL layer-19 bound-async leg [%s] TIMED OUT (socket hang)\n", label);
            std::fflush(stdout);
            std::_Exit(1);
        }
    });
...
// ---------------------------------------------------------------------------------------------
// LEG 1: three clients each fly their own seat through the ASYNC path; the union reproduces
// build_server_frames byte-for-byte.
// ---------------------------------------------------------------------------------------------
static int run_leg1() {
    const Rails rails = sealed_rails();
    const session::Scenario sc = ba_scenario(/*ticks=*/150u);
    ...
    std::vector<ClientPlan> plans = {
        {own_seat_cmds, /*chunk=*/1, /*reverse=*/true},
        {own_seat_cmds, /*chunk=*/7, /*reverse=*/false},
        {own_seat_cmds, /*chunk=*/3, /*reverse=*/true},
    };

    netinput::Stats st;
    std::vector<ClientResult> res;
    run_bound_async_leg(rails, sc, plans, st, res, "leg1/3-clients-async");
    ...
    for (std::size_t i = 0; i < res.size(); ++i) {
        ...
        if (!c.bind_ok || c.bind.n_aircraft != 3 || c.bind.seat < 0 || c.bind.seat >= 3 ||
            seat_seen[static_cast<std::size_t>(c.bind.seat)]) {
            ++fails;
            std::printf("FAIL leg1: client %zu bad/duplicate BIND (ok=%d seat=%lld n=%lld)\n", i,
                        c.bind_ok ? 1 : 0, static_cast<long long>(c.bind.seat),
                        static_cast<long long>(c.bind.n_aircraft));
        } else {
            seat_seen[static_cast<std::size_t>(c.bind.seat)] = true;
        }
    ...
    if (fails == 0)
        std::printf("  LEG1 PASS: 3 clients each flew their OWN seat (scrambled/chunked) through the "
                    "ASYNC path -> %zu frames byte-identical to build_server_frames; distinct BIND "
                    "seats, zero unauthorized/capped/reaped\n", ref.size());
    return fails;
}

// ---------------------------------------------------------------------------------------------
// LEG 2: one client at seat 0 tries to steer EVERY aircraft through the async path; only its own-seat
...
static int run_leg2() {
    ...
    const std::size_t foreign = canon.size() - own0.size();
    const auto ref = run_input_frames(rails, sc, own0);  // only aircraft 0 commanded

    auto send_everything = [&](std::int64_t /*seat*/) { return canon; };
    std::vector<ClientPlan> plans = {{send_everything, /*chunk=*/1, /*reverse=*/true}};

    netinput::Stats st;
    std::vector<ClientResult> res;
    run_bound_async_leg(rails, sc, plans, st, res, "leg2/foreign-reject-async");

    int fails = 0;
    if (!st.ok || st.frames_sent != ref.size() || st.joins != 1) {
        ++fails;
        std::printf("FAIL leg2: server stats (ok=%d sent=%zu/%zu joins=%zu)\n", st.ok ? 1 : 0,
                    st.frames_sent, ref.size(), st.joins);
    }
    if (st.cmds_ok != own0.size() || st.cmds_unauth != foreign || st.cmds_stale != 0 ||
        st.cmds_oob != 0 || st.capped != 0 || st.reaped != 0) {
        ++fails;
        std::printf("FAIL leg2: accounting (ok=%zu unauth=%zu; want %zu own / %zu foreign, 0 hygiene)\n",
                    st.cmds_ok, st.cmds_unauth, own0.size(), foreign);
    }
    const ClientResult& c = res[0];
    if (!c.bind_ok || c.bind.seat != 0 || c.bind.n_aircraft != 3) {
        ++fails;
        std::printf("FAIL leg2: BIND (ok=%d seat=%lld n=%lld; want seat 0 / n 3)\n", c.bind_ok ? 1 : 0,
                    static_cast<long long>(c.bind.seat), static_cast<long long>(c.bind.n_aircraft));
    }
    if (c.pending != 0 || !frames_equal(c.frames, ref)) {
        ++fails;
        std::printf("FAIL leg2: downstream NOT byte-identical to the aircraft-0-only world (got %zu of "
                    "%zu, pending %zu)\n", c.frames.size(), ref.size(), c.pending);
    }
    if (fails == 0)
        std::printf("  LEG2 PASS: a seat-0 client's %zu foreign-aircraft commands were rejected "
                    "(cmds_unauth=%zu) through the async path; downstream is the aircraft-0-only world "
                    "byte-for-byte\n", foreign, st.cmds_unauth);
    return fails;
}

// =============================================================================================
// LEG 3: a dead client is bounded (reaped/capped) + its SEAT freed without disturbing a cooperative
// seated client. FAST (seated, hook-drained) upstreams ONLY its own seat's commands => byte-identical
// downstream (after BIND) to the reference for the seat it drew, its commands drive the sim. DEAD
// connects, sends/reads nothing => dropped by the policy under test, its seat freed (leaves==1).
// `use_liveness` selects the policy (liveness reap at cap=0, or byte-cap at liveness=0).
// =============================================================================================
static int run_hygiene_leg(bool use_liveness, const char* label) {
    const Rails rails = sealed_rails();
    const session::Scenario sc = ba_scenario(/*ticks=*/20000u);  // ~4000 frames, ~1 MB downstream
    const auto canon = netinput::commands_from_scenario(sc);
    ...
    std::thread server([&] {
        ...
        stats = netinput::broadcast_bound_async(listener, producer, q, /*min_initial=*/2,
    ...
    // DEAD: connect, send/read nothing until the broadcast returns (dropped long before), then drain
    ...
    std::thread watchdog([&] {
        ...
        if (done.load() < 2) {
            std::printf("FAIL layer-19 hygiene leg [%s] TIMED OUT (wedge)\n", label);
            std::fflush(stdout);
            std::_Exit(1);
        }
    });
    ...
    // FAST connects on the main thread, reads its BIND, and upstreams ONLY its own seat's commands.
    ...
    // The reference for FAST's downstream: only its own seat commanded (the other aircraft hold neutral).
    const std::size_t n_own = fast_bind_ok
        ? filter_by_aircraft(canon, fast_bind.seat).size() : 0;
    ...
    if (!fast_bind_ok || fast_bind.seat < 0 || fast_bind.seat >= 3 || fast_bind.n_aircraft != 3) {
        ++fails;
        std::printf("FAIL [%s]: FAST BIND (ok=%d seat=%lld n=%lld)\n", label, fast_bind_ok ? 1 : 0,
    ...
    if (fast_pending != 0 || !frames_equal(fast_frames, ref)) {
        ++fails;
        std::printf("FAIL [%s]: FAST's stream (after BIND) not byte-identical to its seat's reference "
                    "(got %zu of %zu frames, pending %zu)\n", label, fast_frames.size(), ref.size(),
                    fast_pending);
    }
    // DEAD's delivered bytes: reassemble; the FIRST record is its BIND (delivered first by the async
    // buffer), the rest a strict prefix of the frame stream (dropped before the end).
    {
        framing::StreamReassembler r;
        ...
        if (!prefix) {
            ++fails;
            std::printf("FAIL [%s]: DEAD's delivery is not [BIND | strict frame-prefix] (bind_ok=%d "
                        "frames=%zu/%zu)\n", label, dead_bind_ok ? 1 : 0, got_frames, ref.size());
        }
    }
    if (fails == 0)
        std::printf("  %s PASS: DEAD dropped by %s (reaped=%zu capped=%zu) + its seat freed; FAST "
                    "(seat %lld) byte-identical to its seat reference + its %zu commands drove the sim; "
                    "DEAD got [BIND | strict prefix]\n", label, use_liveness ? "liveness" : "byte-cap",
                    stats.reaped, stats.capped, static_cast<long long>(fast_bind.seat), n_own);
    return fails;
}
...
int main() {
    ...
    if (fails == 0) {
        std::printf("PASS: layer-19 bound + async server — the layer-18 seat binding (SeatPolicy + "
                    "BIND-001 + authorization) composes with the layer-16 async/byte-cap/liveness "
                    "downstream hygiene: seated clients each fly their own aircraft to build_server_frames "
                    "byte-for-byte through the async path, and a dead client is bounded (reaped/capped) + "
                    "its seat freed without disturbing a cooperative client\n");
        return 0;
    }
    std::printf("RESULT: layer-19 bound-async bridge FAIL (%d mismatches)\n", fails);
    return 1;
}
