...
constexpr int64_t SNAPSHOT_PROTOCOL = 3;  // framing version (>=2 => KIN; >=3 => KIN-002 w/ gamma)
...
// maps to GEO-001 `bearing`; `phi` (bank, rad) -> KIN phi (deg); `gamma` (flight-path angle, rad)
// -> KIN gamma (deg); `tas` passes through (m/s). Transmits faithfully (no lon/heading normalization).
