...
//      gunnery state: per-aircraft n × (id, hp_q, fire_cd_q [, ammo_q] [, last_hit_by_q]
//      [, engine_hp_q, wing_hp_q, tail_hp_q, kills_q]) (hp/fire_cd ×1e3; ammo ×1 iff protocol
//      >= 5, seal v1.14r0; last_hit_by ×1 iff protocol >= 6, seal v1.17r0; the v1.18r0 region
//      sub-pools engine/wing/tail ×1e3 + the kill tally ×1 iff protocol >= 7, seal v1.19r0 —
//      integer-valued fields ride unit scale, exact + compact like ttl/owner), then a
...
constexpr int64_t SNAPSHOT_PROTOCOL = 7;  // (>=2 KIN; >=3 KIN-002 gamma; >=4 WEAPON-001 gunnery; >=5 ammo; >=6 last_hit_by; >=7 region pools + kills)
...
constexpr int64_t ENGINEHP_SCALE = 1000;  // 1e3 (ENGINE region sub-pool, quarter-integer multiples so milli exact; protocol >= 7)
constexpr int64_t WINGHP_SCALE   = 1000;  // 1e3 (WING region sub-pool; protocol >= 7)
constexpr int64_t TAILHP_SCALE   = 1000;  // 1e3 (TAIL region sub-pool; protocol >= 7)
constexpr int64_t KILLS_SCALE    = 1;     // 1e0 (victory tally — integer counter like ammo; protocol >= 7)
...
struct EntityState {
    ...
    double engine_hp = 0.0;  // WEAPON-001 (v1.19r0, protocol >= 7): region sub-pools + kill tally
    double wing_hp = 0.0;
    double tail_hp = 0.0;
    double kills = 0.0;
};
...
// the WEAPON-001 fields (hp/fire_cd/ammo/last_hit_by/engine_hp/wing_hp/tail_hp/kills) pass
// through (kernel units). Transmits faithfully (no normalization).
EntityState from_kernel(int64_t id, double lat_rad, double lon_rad, double psi_rad,
                        ...
                        double engine_hp = 0.0, double wing_hp = 0.0, double tail_hp = 0.0,
                        double kills = 0.0);
