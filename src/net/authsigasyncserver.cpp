// SEADS authoritative ASYMMETRIC-SIGNATURE + ASYNC server (netcode layer 28). See authsigasyncserver.h.
//
// This is broadcast_auth_async's async loop (authasyncserver.cpp) with the token LOOKUP replaced by the
// layer-27 challenge/signature verify: on accept send a fresh CHALLENGE-001 nonce (blocking), read the
// client's HELLO-003 [token, signature] (bounded-blocking), and PubkeyTable::authenticate verifies the
// Ed25519 signature under the enrolled PUBLIC key before binding the designated seat; then go non-blocking,
// ENQUEUE the BIND-001, and run the SAME async send buffers / byte-cap / liveness reap + per-seat
// authorization as layer 22. The sealed broadcast.cpp AND broadcast_bound / broadcast_bidi /
// broadcast_bound_async / broadcast_auth / broadcast_auth_async / broadcast_authmac / broadcast_authsig are
// byte-for-byte UNTOUCHED — a sibling that owns its own client struct + downstream helpers, reusing
// PubkeyTable / seat_authorizes / CHALLENGE-001 / HELLO-003 / BIND-001 verbatim. Transport only: no
// kernel/det_math, no seal, no Stats change.
...
#include "authmac001.h"  // CHALLENGE-001 codec + derive_nonce (freshness, reused from layer 26/27)
#include "authsig001.h"  // HELLO-003 codec (the Ed25519 proof)
#include "bind001.h"
#include "framing.h"
#include "input001.h"
...
// and its resolved SEAT. authasyncserver.cpp's AuthAsyncClient exactly — the stronger credential changes
// only HOW the seat is resolved (a verified signature vs a bare token lookup).
struct AuthSigAsyncClient {
    netsock::socket_t s;
...
bool flush_client(AuthSigAsyncClient& c) {
...
bool over_cap(const AuthSigAsyncClient& c, std::size_t cap_bytes) {
    return cap_bytes > 0 && c.pending_bytes() > cap_bytes;
}

bool enqueue_bytes(AuthSigAsyncClient& c, const std::vector<std::uint8_t>& bytes) {
    c.buf.insert(c.buf.end(), bytes.begin(), bytes.end());
    return flush_client(c);
}

// Drop client i: return its seat to the PubkeyTable (so a reconnecting identity reclaims it), close, erase,
// count the leave. authasyncserver.cpp::drop_client verbatim, on the public-key roster.
void drop_client(std::vector<AuthSigAsyncClient>& clients, std::size_t i, Stats& st, PubkeyTable& creds) {
...
void reap_dead(std::vector<AuthSigAsyncClient>& clients, Stats& st, std::size_t liveness_frames,
               PubkeyTable& creds) {
    if (liveness_frames == 0) return;
    for (std::size_t i = clients.size(); i-- > 0;) {
        AuthSigAsyncClient& c = clients[i];
...
Stats broadcast_authsig_async(netsock::socket_t listener, InputProducer& producer, CommandQueue& queue,
    ...
    Stats st;
    std::vector<AuthSigAsyncClient> clients;
    ...
    // still blocking; a cooperative client reads it first); (b) read its HELLO-003 (the FIRST upstream
    // framing frame, bounded-blocking); (c) VERIFY the signature to a DESIGNATED seat (or spectator); (d) go
    // non-blocking and ENQUEUE the BIND-001 as the first bytes of its send buffer (after the CHALLENGE).
    // Any command frames pipelined AFTER the HELLO in the same read are submitted immediately (authorized).
    // A failed challenge send, missing/malformed HELLO, or fatal BIND flush drops the joiner (its seat, if
    // verified, is returned).
    auto accept_all = [&]() {
        while (true) {
            netsock::socket_t c = netsock::accept_one(listener);
            if (!netsock::is_valid(c)) break;
            AuthSigAsyncClient ac;
            ac.s = c;
            ...
            // (b) read upstream until the HELLO-003 record (the first framing frame) completes (bounded).
            ...
            // (c) VERIFY the signature to a seat (or spectator); (d) go non-blocking; enqueue the BIND.
    ...
    if (clients.size() < min_initial) {
        for (AuthSigAsyncClient& c : clients) netsock::close_socket(c.s);
        return st;  // ok stays false
    }

    // --- bidirectional frame loop (authasyncserver.cpp's loop verbatim) -------------------------
    ...
    for (std::size_t fi = 0;; ++fi) {
        ...
        rfds.clear();
        wfds.clear();
        for (const AuthSigAsyncClient& c : clients) {
            rfds.push_back(c.s);
    ...
    // --- bounded DRAIN (authasyncserver.cpp's tail verbatim) ------------------------------------
    int idle = 0;
    while (idle < 600) {
        rfds.clear();
        wfds.clear();
