...
| `viewer_main.cpp` → **`seads_viewer`** | OPTIONAL raylib 3D globe (built only with `-DSEADS_CLIENT=ON`). Replay mode: wall-clock render-interpolation, trails, HUD, orbit camera, `--selfcheck N` headless data-path mode. **`--fly` mode:** own ship flown live (A/D bank, W/S climb) via a fixed-100 Hz `predict::Predictor` driving the real sealed kernel; remotes stay on the layer-4a interp path. `--fly --selfcheck N` is a headless (no-GPU, no-recording) proof of the input→prediction path. Links `seads_predict` for fly mode. Both HUDs surface the flight-model atmosphere presentation-side: **`sig`** = ISA density ratio σ(alt) (v1.21r0) and **`pwr`** = supercharger lapse min(1, σ/σ_crit) (v1.22r0); the scoreboard adds static airframe data — region **toughness in eighths** (v1.20r0, recovered from the wire's first-frame pools) and **crit alt** (from the v3 type trailer's envelope). |
| `web/` | Zero-build Three.js globe viewer. Same interpolation (mirrored in JS), opens in any browser. HUD rows carry the same σ(alt) readout (ICAO law, presentation-only). |
