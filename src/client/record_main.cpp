...
#include "snapshot.h"
#include "seadsrec.h"
#include "aircraft_mesh.h"  // AircraftType presentation codes (.seadsrec v3 type trailer)
...
uint32_t type_code_of(const Envelope* e) {
    ...
    if (e == &envtab::P47D) t = AircraftType::P47D;
...
void write_js(const std::string& path, const client::RecordingMeta& meta, const std::string& id,
    ...
    std::fprintf(fp, "  \"meta\": {\"scenario\": \"%s\", \"radius_m\": %.1f, \"tick_hz\": %u, "
                     "\"snap_hz\": %u, \"protocol\": %u, \"n_frames\": %u, \"types\": [",
                 id.c_str(), meta.radius_m, meta.tick_hz, meta.snap_hz, meta.protocol,
                 static_cast<unsigned>(frames.size()));
    // Airframe display names, index-aligned with aircraft slot/id (presentation metadata).
...
int main(int argc, char** argv) {
    ...
    // Airframe types (presentation metadata, .seadsrec v3 trailer + trajectory.js meta).
    std::vector<uint32_t> types;
    for (const auto& a : flight.ac) types.push_back(type_code_of(a.env));
    ...
    // Combat summary (final kernel state): airframe type + hp per aircraft + any kills.
    for (std::size_t a = 0; a < flight.ac.size(); ++a)
        std::printf("  aircraft %zu (%s): hp %.0f%s\n", a,
                    client::aircraft_type_name(client::aircraft_type_from_code(types[a])),
                    k.hp(a), k.hp(a) <= 0.0 ? "  *** KILLED ***" : "");
    std::printf("  rounds airborne at end: %zu\n", k.proj_count());
    std::printf("  combat events (per-round journal): %zu\n", events.size());

    if (out_path) {
        std::vector<uint8_t> blob;
        client::write_recording(meta, wire_frames, events, types, blob);
    ...
    if (js_path) {
        write_js(js_path, meta, flight.id, decoded_frames, types);
