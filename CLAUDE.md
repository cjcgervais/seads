...
- next — free pick (none blocking): **layer 23 — authenticated + catch-up** (the last rung of the
  authenticated arc, 21→22→23: fold layer-20's windowed catch-up onto `broadcast_auth_async` — mechanical:
  catch-up = replay-depth, orthogonal to admission + delivery; this server now owns the send buffers a
  catch-up prefix enqueue builds on); **stronger credentials** (a real MAC/signature the server verifies vs
  the abstracted i64 token); input prediction of REMOTE aircraft (predict-others, not just layer-4a
  interpolate); or renderer polish (surface the assigned seat / auth state / the predicted-vs-authoritative
  correction / catch-up-in-progress on the HUD).
