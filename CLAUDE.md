...
- next — free pick (none blocking): put the region pools + kills on the WEAPON-001 wire (a
  transport-only reseal, the proven ammo/last_hit_by pattern ⇒ a client draws damage state + a
  scoreboard); renderer polish (meshes; guns + kill-feed in the live `--fly` path — per-round
  impact/damage/region data now exact); **B5** ISA atmosphere (a seal); or an open-ended live
  frame SOURCE feeding `broadcast_async` incrementally.
