...
- next — free pick (none blocking): per-round hit granularity (a kernel event queue, its own ADR); renderer
  polish (meshes; guns + kill-feed in the live `--fly` path); an optional new seal (component-damage,
  **B5** ISA atmosphere); or an open-ended live frame SOURCE feeding `broadcast_async` incrementally.
