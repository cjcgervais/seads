...
- next — free pick (none blocking): input prediction/reconciliation against the authoritative input
  server (client applies its own commands locally, rewinds/replays on the authoritative frame); or
  bidirectional late-join catch-up once the client→aircraft binding is settled; or more renderer polish.
