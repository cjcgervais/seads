...
| Wire/Hash | **GEO-001** — lat/lon×1e7, bearing×1e6, h×1e3; ZigZag+LEB128. **+KIN-002** aux block (phi×1e6, tas×1e3, **gamma×1e6**) for prediction. **+WEAPON-001** aux block (per-aircraft hp×1e3, fire_cd×1e3, **ammo×1e0**, **last_hit_by×1e0**; per round: GeoPoint + damage×1e3 + ttl/owner exact i64) for MP gunnery replication; snapshot **protocol 6** (WEAPON-001 v1.12r0; `ammo` added v1.14r0, protocol 4→5; `last_hit_by` added v1.17r0, protocol 5→6). The wire is lossy/downstream — `Kernel::snapshot()` stays the world_hash source of truth |
...
- **Netcode layers 8–11 (no-seal, ride v1.17r0):** the socket transport grows into a real fan-out server,
  ...
  pinned tiny kernel buffers where a blocking server provably wedges (`seads_netasync_test`).
  See ADR-Step-Net-Layer{8,9,10,11}-*-v1.17r0.
- next — free pick (none blocking): a send-buffer byte-cap + drop-slowest policy (live-stream hygiene);
