...
- next — free pick (none blocking): **B5** ISA atmosphere (a seal); an open-ended live frame SOURCE
  feeding `broadcast_async` incrementally; or per-airframe region toughness follow-ups (e.g. an
  A6M2 engine retune — its own data-only seal that moves EngineOut-001's story).
