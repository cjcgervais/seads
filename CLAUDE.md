...
- next — free pick (none blocking): a real bidirectional server merging the layer 11–15a output
  hygiene (async / byte-cap / liveness) with the upstream input path; or input
  prediction/reconciliation against the authoritative input server; or more renderer polish.
