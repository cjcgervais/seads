...
- next — free pick (none blocking; the authenticated arc is COMPLETE — the four transport axes admission ×
  delivery × replay-depth with identity binding are all composed): **stronger credentials** (a real
  MAC/signature the server verifies vs the abstracted i64 token); input prediction of REMOTE aircraft
  (predict-others, not just layer-4a interpolate); or renderer polish (surface the assigned seat / auth
  state / the predicted-vs-authoritative correction / catch-up-in-progress on the HUD).
