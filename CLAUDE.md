...
- next — free pick (none blocking): **authenticated binding** (identity, not just join-order position —
  the last honest-scope caveat on the binding); input prediction of REMOTE aircraft (predict-others, not
  just layer-4a interpolate); or renderer polish (surface the assigned seat / the predicted-vs-authoritative
