...
- next — free pick (none blocking): **key rotation / revocation / a real PKI** (certificates over the
  enrolled public keys, the honest-scope follow-up to layer 27's fixed roster — the asymmetric arc's named
  next hardening); or renderer polish (surface the assigned seat / auth state / the
  predicted-vs-interpolated-vs-smoothed remote / the correction magnitude / catch-up-in-progress on the HUD).
