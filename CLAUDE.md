...
- next — free pick (none blocking): an A6M2 engine-toughness retune (data-only seal that moves
  EngineOut-001's story); per-airframe toughness / σ / crit-alt surfaced in the HUD
  (presentation-only); projectile σ-drag (a kernel seal that moves every firing golden,
  deliberately deferred at B5); or a per-airframe two-speed blower schedule (deferred at
