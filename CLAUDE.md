...
- next — free pick (none blocking): **CA-certificate folded onto the async & catch-up servers** (re-run the
  27→28/29 step on the certificate credential: `broadcast_authcert_async` + `broadcast_authcert_catchup`);
  or **certificate chains / intermediate CAs** (path validation up to a trusted root — the honest-scope
  follow-up to layer 30's single self-signed root); or renderer polish (surface the assigned seat / auth
  state / the predicted-vs-interpolated-vs-smoothed remote / the correction magnitude / catch-up-in-progress
  on the HUD).
