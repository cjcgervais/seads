...
| Atmosphere | **Still air** — no wind / Coriolis / weather. **B5 (v1.21r0): ISA density-vs-altitude** — σ(alt)=ρ/ρ₀, a sealed 17-node LUT (500 m spacing, ICAO troposphere provenance, `lut_eval` interp ⇒ zero new det_math) scaling BOTH `q=½ρ₀σV²` AND thrust `T×=σ` in the envelope-driven step; no-arg path + projectile drag untouched |
...
- next — free pick (none blocking): per-airframe supercharger critical altitude (B5's named
  data-driven follow-up — thrust lapse per airframe, its own seal); an A6M2 engine-toughness
  retune (data-only seal that moves EngineOut-001's story); per-airframe toughness / σ surfaced
  in the HUD (presentation-only); or projectile σ-drag (a kernel seal that moves every firing
  golden, deliberately deferred at B5).
