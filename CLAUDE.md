...
- next — free pick (none blocking): renderer polish (meshes; guns + kill-feed in the live `--fly` path —
  per-round impact/damage data now exact); an optional new seal (component/region damage — the natural
  consumer of per-round events; **B5** ISA atmosphere); or an open-ended live frame SOURCE feeding
  `broadcast_async` incrementally.
