...
- next — free pick (none blocking): predict-others reconciliation SMOOTHING (blend the coast toward each
  reseed instead of snapping, to hide the maneuver correction — pure presentation on top of layer 24);
  **stronger credentials** (a real MAC/signature the server verifies vs the abstracted i64 token); or
  renderer polish (surface the assigned seat / auth state / the predicted-vs-interpolated remote / the
