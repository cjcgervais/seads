...
- next — free pick (none blocking): bounded/windowed catch-up for open-ended live streams (layer
  13's named boundary); per-airframe supercharger critical altitude (B5's named data-driven
  follow-up — thrust lapse per airframe, its own seal); an A6M2 engine-toughness retune (data-only
  seal that moves EngineOut-001's story); or per-airframe toughness surfaced in the HUD
  (presentation-only).
