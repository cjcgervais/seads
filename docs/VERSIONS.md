...
| *(v16 candidate — NOT sealed)* | `feel/lateral-yawbudget` @ `40325f297` | S-LATERAL: `[coordination] yaw_vert_budget` 1.0 + `unload_below_horizon` 1.0, the lateral nose-down (TARGET 2). Flown twice, liked ("I like it now"), red-team LAND-WITH-FIX folded `b4864512c`, **ON HOLD for Chad's ruling** (the unload arms in the back half of a vertical loop). See `docs/cascade/lateral-nose-down-unload.md`. |
