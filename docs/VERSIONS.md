...
| `sled-kernel-v3-gated` | `babf9df6a` | 2026-09-22 — SLED KERNEL v3 CANDIDATE, GATED NOT DRIVEN: N2 `ice_bite_mu` 0.25 (lake-ice low-speed ski bite) + N1 `leg_work_nm` 2400 (CTRL/SHIFT leg work), one dial each, identity 0.0 = v2 bit-exactly, env kill switches. Landed push-before-drive on Chad's sentence "please add the docs and addendum and these additions to the sled kernel ty ... then push to main and seads-recon". Gate on `c8a61c05c` 6 of 2165 by name; red-team four P1s folded. His drive word per rung + two named rulings OWED; `-signed` follows. `reference/seads-feel/` re-snapshotted here. |
