...
// RE-RECORDED 2026-09-10 (S-leanlead, feel/yaw-bank-balance sandbox): [auto_level]
// lean_lead 0.3 -- the FINE wings-hold roll limb now targets the live lean on the
// way in (test_yawbank_balance.cpp pins the mechanism + the bit-identical arms).
// The knob-off arm (lean_lead 0) is the pre-record tree bit-identically; the moved
// checkpoints are the roll channel and its downstream carry. Intentional.
// RE-RECORDED 2026-09-12 (S-leanlead-lateral, feel/lean-lead-walkback): the
// [auto_level] lead is restored to its flown 0.3 but now GATED by the aim
// offset's horizon-lateral share (lean_lead_lateral, controller.cpp), so a
// pure-pitch pull no longer leads the wings into a bank. Checkpoints 300 and
// 600 came back BIT-IDENTICAL to the v14 table (b3bd942a3) -- the scripted
// flight's lateral legs read share ~ 1, the gate is fully open there and the
// flown turn-entry feel is untouched. 900 and 1200 moved: those legs carry
// more vertical aim, the gate partially closes, and the roll channel (plus
// its downstream carry) takes the pre-v14 value instead. Both knob-off arms
// were verified bit-identical FIRST (test_yawbank_balance.cpp pins
// lean_lead = 0, and lean_lead_lateral = false, tick for tick). Intentional.
// RE-RECORDED: 2026-07-05 — §7 feel tunes (Cl_max 1.4->1.8; K_coord 2.0->3.0).
// MERGED 2026-07-17 (v4 flight kernel x Scarce Skies R6, merge c08651a16): the
// v4 table is retained VERBATIM (the pool-ball capture recorded it); R6's
// spatial-density seam is null-env bit-identical by contract, so the merged
// kernel reproduces this exact table at env=null — gate-proven, NOT re-recorded
// (nothing re-recorded off the merge = no re-record-blesses-the-bug risk).
...
// RE-RECORDED 2026-07-30 (S-STRAIGHTLINE — the tracer-line dip, [regime]
// line_hold_ff 0.0 -> 1.0): the axis-correction pitch FF cancels the emitted
// yaw's PARASITIC vertical component (the part moving the nose AWAY from the
// aim's elevation line: -parasitic/cosPhiTheta with parasitic =
// w_dn*min(yv,0) + w_up*max(yv,0), yv = yaw*sin(phi), one-sided per side of
// the line over a +/-0.087 sag band; gated blend*fwd_gate*knife_fade) so the
// pointing sweep to the aim goes about LOCAL UP instead of the banked body
// axis (docs/straightline_thread.md — the CRAB attribution; the approved
// gravity shape was killed by the instrument at 2-4% closure before any fly,
// and the v2 all-component cancel was killed by the additivity premise sweep
// — it drove elevated-aim-while-banked geometries into the -G floor).
// KNOB-OFF ARM PROVEN FIRST: with line_hold_ff = 0.0 the prior (S-rollmix)
// golden reproduced (all 83 assertions) — structural off, the FF block is
// skipped entirely; nothing but this mechanism moved. Divergence signature
// VERIFIED before this record (per-tick knob-0-vs-knob-1 probe): FIRST
// DIVERGENT TICK 240 — the EXACT tick the scripted 25 deg lateral snaps in
// (err 25 deg, blend 1, bank -6.4e-15 deg): the FF's first add is fp-tiny at
// near-zero bank (movers equal to 6 sig figs on tick 240) and grows with the
// bank; every tick through 239 (the whole level-cruise prefix) BIT-IDENTICAL.
// Checkpoint-300 signature: the capture completes FASTER under the level
// sweep — regime already FINE at t300 (was MANEUVER), held_bank 0.853
// captured (was ~0), integ_x 0.77 vs 2.10 (less error carried through the
// entry). NOTE (re-audit P2-4): S-rimshot CARRY/RETURN would overwrite the FF
// wholesale, but the machine is PARKED (carry = 0) — no capture-owned ticks
// exist in this script. Walk-back = line_hold_ff 0.0 (the fly kill-switch;
// bit-identical v11 tree). Intentional.
inline constexpr CtrlCheckpoint kCtrlFlight[] = {
    {300,
     17996.757272543364, -2.7582142058039789, -349.65619811098105,
     -0.89993056179729836, -17.311041944164305, -138.46162492863851,
     0.26195749573244953, -0.047030893502584971, -0.18465146995791926, -0.94608149761440319,
     0.25718566509707297, -0.051048508169110961, 0.76784444453431311,
     0.0092086657346033031, 0.022422760902396685, 0.071744531055919353, 0.85300745695419256, 0},
    {600,
     17992.165705612271, -113.93726121496789, -676.88680787846909,
     -5.4325285184567189, -55.019123975319452, -126.84309678385071,
     0.69147993978152644, -0.15549239910341603, -0.15031930947179445, -0.68926171509864331,
     0.10810862202127186, 0.0039220532009318302, 0.0028130054789127202,
     -0.0015584525078269644, 0.019940761750004331, 0.032334136286245645, 0.0055222913999368323, 1},
    {900,
     18055.363531353745, -250.47917044621212, -979.14827163650546,
     36.376552143044307, -53.199680997642169, -115.44169410736231,
     0.70893507120259025, -0.041480879127968974, -0.26023720131503475, -0.65419186829053999,
     -0.0059159788338617901, -6.277524536435551e-05, 0.0011178631419886164,
     -0.029966911295127151, 0.015727227339973875, 0.025668738711019566, 0.0020414643177021092, 0},
    {1200,
     18141.779402748296, -380.31913492799134, -1259.6488762977142,
     32.369238398034724, -50.653925947530055, -109.17969634845156,
     0.70878352910835096, -0.047834844811158655, -0.25433432619805751, -0.65624064717448249,
     -0.006013117909365602, -2.3455559408379118e-05, 0.00011697439118379285,
     -0.013499226006672763, 0.012432838545761864, 0.020260455645844517, 0.0014380252468613754, 0},
    // final: alt 3189.43 m, speed 124.635 m/s
};
