...
| AT-7 | Override-release catch — **SUPERSEDED (SPEC §0 S7-ovr/S7-ovr2):** override is IN-envelope and never touches the aim | re-scoped: the aim/reticle does not move under a held override (the mouse owns the aim; keyboard is a pure in-envelope nudge); release: pursuit resumes, φ_held recaptured, braking law flies the nose back to the held aim |
| AT-8 | Freelook compose, no snap-back | freelook + keyboard 180° roll, release both: commanded Inputs continuous; release step ≤ budget; ~~target ≡ nose~~ **SUPERSEDED (SPEC §0 S7-nest): during the hold the aim RIDES THE NOSE per tick; on release (override used) the aim snaps to the guarded VELOCITY (caller's own `normalize(curr.velocity)`; below `v_ballistic` or tail-slide → nose)**; ease ≤300 ms outside loop; **no transient inverted mouse control during ease-back (released looking aft) — CQ2-invariant: holds whichever mechanism (suspend vs easing-frame) is ruled** |
...
| AT-11 | AoA protection | full-up at low speed + injected inverted-stall AoA: no departure; protection obeys both G floor and ceiling; **inactive at cruise AoA**; ~~override can exceed (by design)~~ **SUPERSEDED (SPEC §0 S7-ovr): override respects the SAME AoA/G protection — the escape-hatch is removed** |
